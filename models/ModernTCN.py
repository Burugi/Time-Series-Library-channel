...
class ModernTCN(nn.Module):
    def __init__(self,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
        ...
        # head
        # Calculate actual patch_num considering padding in stem layer
        if patch_size > patch_stride:
            padded_seq_len = seq_len + (patch_size - patch_stride)
        else:
            padded_seq_len = seq_len

        # Conv1d output size: (input - kernel_size) // stride + 1
        patch_num = (padded_seq_len - patch_size) // patch_stride + 1
        ...
        if use_multi_scale:
            ...
            # Calculate final feature map size after all downsampling
            final_patch_num = patch_num
            for i in range(1, self.num_stage):
                # Each downsample stage
                if final_patch_num % downsample_ratio != 0:
                    final_patch_num = final_patch_num + (downsample_ratio - final_patch_num % downsample_ratio)
                final_patch_num = final_patch_num // downsample_ratio
    ...
    def up_sample(self, x, upsample_ratio):
        _, _, _, N = x.shape
        return F.interpolate(x, size=N, scale_factor=upsample_ratio, mode='bilinear')
