...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        patch_scales = getattr(configs, 'patch_scales', [8, 16, 24, 32])
        stride_scales = getattr(configs, 'stride_scales', [8, 8, 7, 6])

        self.patch_len1 = patch_scales[0]
        self.patch_len2 = patch_scales[1]
        self.patch_len3 = patch_scales[2]
        self.patch_len4 = patch_scales[3]

        self.stride1 = stride_scales[0]
        self.stride2 = stride_scales[1]
        self.stride3 = stride_scales[2]
        self.stride4 = stride_scales[3]
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        actual_patch_dim = x_patch_c.shape[-1]
        expected_patch_dim = self.d_model * self.patch_num1

        if actual_patch_dim != expected_patch_dim:
            x_patch_c = nn.functional.interpolate(
                x_patch_c,
                size=expected_patch_dim,
                mode='linear',
                align_corners=False
            )
        ...
        # Ensure encoding_1_ch has the correct d_model dimension
        if encoding_1_ch.shape[-1] != self.d_model:
            b, c, actual_d = encoding_1_ch.shape
            encoding_1_ch = encoding_1_ch.reshape(b * c, actual_d)
            encoding_1_ch = nn.functional.adaptive_avg_pool1d(
                encoding_1_ch.unsqueeze(1),
                self.d_model
            ).squeeze(1).reshape(b, c, self.d_model)
